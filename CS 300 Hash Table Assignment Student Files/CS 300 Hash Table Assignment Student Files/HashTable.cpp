 ...
 */
HashTable::HashTable() {
    // FIXED: Initialize the structures used to hold bids
 ...
 */
HashTable::~HashTable() {
    // FIXED: Implement logic to free storage when class is destroyed
...
void HashTable::PrintAll() {
    // FIXED (6): Implement logic to print all bids
    // for node begin to end iterate
    for (unsigned int i = 0; i < nodes.size(); ++i) {
        Bid bid = nodes[i].bid;
        Node* nextNode = &(nodes.at(i));
        //   if key not equal to UINT_MAX
        if (i != UINT_MAX) {
            // output key, bidID, title, amount and fund
            cout << "Key " << i << ": " << bid.bidId << " | " << bid.title 
